-- Function: public.insdlo_new(character varying, date, character varying, character varying, character varying, character varying, character varying)

-- DROP FUNCTION public.insdlo_new(character varying, date, character varying, character varying, character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION public.insdlo_new(cma_kho character varying, dto date, cma_vt character varying, cstt_rec character varying, cma_lo character varying, cma_vi_tri character varying, cdvt character varying)
...
DECLARE
	...
	-- SELECT * FROM insdlo_new('KHO01', '20170321', 'BTP-HC-00001', '', '', '', 'TO') 
BEGIN
	...
	SELECT a.ma_vt, ''::character varying ten_vt, a.ma_lo, ''::character varying ten_lo, ''::character varying ten_lo2, 
		...
		sum(a.so_luong2) as so_luong2, ''::character varying po_khsx, ''::character varying dau_tuan
	FROM(
	...
	FOR r IN select * from tmp ORDER BY ngay_nhap
        LOOP
		RETURN NEXT r;
	END LOOP;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION public.insdlo_new(character varying, date, character varying, character varying, character varying, character varying, character varying)
  OWNER TO postgres;
-- SELECT * FROM insdlo_new('', '20230428', '00181557', '', '', '', 'CHIEC')